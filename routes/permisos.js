...
[empresa?.trim()||null, contrato?.trim()||null, responsable_contrato||null, responsable1||null, responsable2||null, responsable1_tel||null, responsable2_tel||null, fecha_inicio||null, fecha_fin||null, user.rol === 'contratista' ? null : user.id||null, ip_creacion||null, firma_creacion_ip_privada||null, firma_creacion_ubicacion||null, usuario_creacion, es_pase_visita === true]
    ...
    if (sec.personal&&Array.isArray(sec.personal)) { for (const p of sec.personal) { if (!p.nombre&&!p.num_credencial) continue; await pool.query(`INSERT INTO permiso_personal (permiso_id, num_credencial, nombre, categoria, observaciones, documento, documento_nombre_extraido, documento_validado) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`, [pid, p.num_credencial||null, p.nombre||null, p.categoria||null, p.observaciones||null, p.documento_ine||p.documento||null, p._nombreExtraido||null, p._docInlineValidado===true]); } }
    ...
    if (sec.vehiculo&&Array.isArray(sec.vehiculo)) { for (const v of sec.vehiculo) { if (!v.marca&&!v.placas) continue; const segExt=v.seguro_extracted||{}, licExt=v.licencia_extracted||{}; const tarExt = v.tarjeta_extracted || {};
...
[pid, v.marca||null, v.modelo||null, v.placas||null, v.seguro||null, v.licencia||null, v.tarjeta||null, segExt.numero_poliza||null, segExt.aseguradora||null, segExt.vigencia_inicio||null, segExt.vigencia_fin||null, segExt.vigente??null, licExt.nombre_conductor||null, licExt.numero_licencia||null, licExt.tipo_licencia||null, licExt.vigencia_fin||null, licExt.vigente??null, tarExt.numero_serie||null, tarExt.placas||null, tarExt.vigencia_fin||null, tarExt.vigente??null]); } }
...
router.get('/verificar-personal', requireAuth, async (req, res) => {
  ...
  try {
    const r = await pool.query(
      `SELECT pp.nombre, p.folio, p.empresa, p.estado, p.fecha_fin
